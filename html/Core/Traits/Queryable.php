...
trait Queryable
{
    ...
    {
        static::resetQuery();  //позднее статическое связывание Car extends Model, Model use Queryable, $tableName cars
        static::$query = "SELECT " . implode(', ', $columns) . " FROM " . static::$tableName . " ";

        $obj = new static(); // статическое позднее связывание new Car()
        $obj->commands[] = 'select';
